(() => {
  ...
  function recordFrom(value){const stamp=now();return {id:uid(),createdAt:stamp,updatedAt:value?.updatedAt||stamp,revision:0,data:normalize(value)}}
  function recordFromServer(value){return {id:value.id,createdAt:value.createdAt,updatedAt:value.updatedAt,revision:Number(value.revision)||0,createdBy:value.createdBy||'',updatedBy:value.updatedBy||'',data:normalize(value.data)}}
  ...
  function readDraft(id){const draft=readDrafts()[id];return draft&&typeof draft==='object'?draft:null}
  function writeDraft(id,data,revision){try{const drafts=readDrafts();drafts[id]={baseRevision:Number(revision)||0,data:clone(data),savedAt:now()};localStorage.setItem(DRAFT_KEY,JSON.stringify(drafts));return true}catch{return false}}
  function clearDraft(id){try{const drafts=readDrafts();delete drafts[id];localStorage.setItem(DRAFT_KEY,JSON.stringify(drafts))}catch{}}
  async function requestJSON(url,options={}){const response=await fetch(url,{...options,headers:{'Content-Type':'application/json',...(options.headers||{})}});let payload={};try{payload=await response.json()}catch{}if(!response.ok){const error=new Error(payload?.detail?.message||payload?.detail||`请求失败（${response.status}）`);error.status=response.status;error.payload=payload;throw error}return payload}
  async function refreshLibrary(){const payload=await requestJSON('/api/bootstrap');currentUser=payload.user||null;library={schemaVersion:'selfit-report-library/1.0',seedVersion:SEED_VERSION,activeId:'',templates:(payload.templates||[]).map(recordFromServer)};renderLibrary()}
  ...
  function viewAsset(source){return typeof source==='string'&&source.startsWith('/api/')?`.${source}`:source}
  ...
  function setEditorState(state,text){saveState.dataset.state=state;saveState.textContent=text;saveButton.disabled=!['dirty','error'].includes(state)}
  function markDirty(){isDirty=true;if(hasConflict){setEditorState('conflict','保存冲突 · 请重新进入');return}if(!writeDraft(editingId,config,baseRevision)){setEditorState('error','草稿暂存失败 · 请立即保存');return}setEditorState('dirty','编辑中 · 尚未保存')}
  async function showLibrary(){if(isDirty&&!window.confirm('当前修改尚未保存，确定放弃并返回模板库吗？'))return;if(isDirty)clearDraft(editingId);config=null;editingId='';baseRevision=0;isDirty=false;hasConflict=false;$('#editorView').hidden=true;$('#libraryView').hidden=false;$('[data-editor-actions]').hidden=true;$('[data-library-actions]').hidden=false;window.scrollTo({top:0,behavior:'smooth'});try{await refreshLibrary()}catch(error){showToast(`数据加载失败：${error.message}`)}}
  function openEditor(id){const record=library.templates.find(item=>item.id===id);if(!record)return;editingId=id;library.activeId=id;baseRevision=Number(record.revision)||0;const draft=readDraft(id);const canRestore=draft&&Number(draft.baseRevision)===baseRevision;config=normalize(canRestore?draft.data:record.data);isDirty=Boolean(canRestore);hasConflict=false;setEditorState(canRestore?'dirty':'editing',canRestore?'已恢复本地草稿 · 尚未保存':'编辑中 · 尚未修改');$('#libraryView').hidden=true;$('#editorView').hidden=false;$('[data-library-actions]').hidden=true;$('[data-editor-actions]').hidden=false;renderForm();renderPreview();window.scrollTo({top:0,behavior:'smooth'})}
  ...
  async function saveCurrent(){
    if(!config||!editingId)return false;if(!isDirty){setEditorState('saved','已保存 · 无新修改');return true}setEditorState('saving','正在保存到后台…');
    try{const payload=await requestJSON(`/api/templates/${encodeURIComponent(editingId)}`,{method:'PUT',body:JSON.stringify({expectedRevision:Number(baseRevision)||0,data:config})});const committed=recordFromServer(payload.template);const index=library.templates.findIndex(item=>item.id===editingId);if(index>=0)library.templates[index]=committed;else library.templates.unshift(committed);config=clone(committed.data);baseRevision=committed.revision;isDirty=false;hasConflict=false;clearDraft(editingId);renderForm();renderPreview();setEditorState('saved',`保存成功 · ${payload.savedBy?.username||payload.savedBy?.email||'已生效'}`);showToast('修改已保存到后台');return true}catch(error){if(error.status===409){hasConflict=true;setEditorState('conflict','保存冲突 · 请重新进入');showToast('另一位编辑者已先保存，请返回模板库重新进入');return false}setEditorState('error',`保存失败 · ${error.message}`);showToast('保存失败，草稿仍保留在当前浏览器');return false}
  }
  ...
  $('#templateList').addEventListener('click',event=>{const button=event.target.closest('[data-action]');if(!button)return;const id=button.closest('[data-template-id]').dataset.templateId;const record=library.templates.find(item=>item.id===id);if(button.dataset.action==='edit')openEditor(id);if(button.dataset.action==='duplicate'){const copy=recordFrom(record.data);copy.data.name=`${record.data.name||'未命名模板'} 副本`;library.templates.unshift(copy);openEditor(copy.id);markDirty();showToast('副本已进入草稿，保存后生效')}if(button.dataset.action==='export')exportRecord(record);if(button.dataset.action==='delete'){pendingDeleteId=id;$('#deleteDialogCopy').textContent=`“${record.data.name||'未命名模板'}”删除后会从模板库隐藏，请先确认已导出需要保留的配置。`;$('#deleteDialog').showModal()}});
  $('#deleteDialog').addEventListener('close',async event=>{if(event.target.returnValue!=='confirm'||!pendingDeleteId)return;const id=pendingDeleteId;const record=library.templates.find(item=>item.id===id);pendingDeleteId='';try{if(record?.revision){await requestJSON(`/api/templates/${encodeURIComponent(id)}`,{method:'DELETE',body:JSON.stringify({expectedRevision:record.revision})})}clearDraft(id);library.templates=library.templates.filter(item=>item.id!==id);selected.delete(id);renderLibrary();showToast('模板已从后台模板库隐藏')}catch(error){showToast(`删除失败：${error.message}`)}});
  function createTemplate(){const record=recordFrom(blankTemplate());library.templates.unshift(record);openEditor(record.id);markDirty();showToast('已新建空白草稿，保存后生效')}
