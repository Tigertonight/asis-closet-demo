(() => {
  ...
  const setLoadingProgress = (progress) => {
    ...
    const nextSrc = `../../static/selfit/assets/loading-stage-${stage.art}@4x.png`;
    if (!art.src.endsWith(nextSrc)) {
      art.src = nextSrc;
      art.animate?.([
        { opacity: .28, transform: 'translateY(4px) scale(.985)' },
  ...
  const DEFAULT_REPORT_DATA = Object.freeze({
    eyebrow: 'LACE',
    title: '造梦浪漫型人',
    traits: ['柔和线条', '浪漫感', '细节表达'],
    summary: '把浪漫穿得轻一点，你的温柔自带分寸。你喜欢有分寸的浪漫：线条柔和、色彩安静，细节精致但不过度用力。',
    illustration: { imageUrl: '../../static/selfit/assets/report-style-soft-cool@4x.png', alt: '造梦浪漫型人风格主题插图' },
    colors: [
    ...
    outfitSummary: '你更容易被柔和轮廓和经典氛围打动。比起醒目的潮流元素，你更在意颜色、材质和细节之间是否舒服。你的浪漫并不繁复，而是藏在自然的曲线、低对比配色和一个恰到好处的装饰里。',
    outfits: [
      { name: '新中式穿搭', byline: '@索贝', imageUrl: '../../static/selfit/assets/figma-report/outfit-01@4x.png', alt: '新中式穿搭参考' },
      { name: '柔雾叠穿', byline: '@索贝', imageUrl: '../../static/selfit/assets/figma-report/outfit-02@4x.png', alt: '柔雾叠穿参考' },
      { name: '古镇穿搭', byline: '@索贝', imageUrl: '../../static/selfit/assets/figma-report/outfit-03@4x.png', alt: '古镇穿搭参考' },
      { name: '柔雾叠穿', byline: '@索贝', imageUrl: '../../static/selfit/assets/figma-report/outfit-02@4x.png', alt: '柔雾叠穿补充参考' },
    ],
    adviceIntro: '你并不是不喜欢表达，只是不喜欢廉价的表达。比起用夸张单品吸引注意，你更擅长把态度藏在面料、领口、色彩关系和一个小小的细节里。',
    advice: ['柔和轮廓：你更适合自然垂坠、轻微收腰和带弧度的线条。', '简约表达：不需要堆叠很多元素，一个细节重点就足够。', '经典耐看：比起追逐短期爆款，你更适合耐看、有质感的单品。'],
  });

  const buildMockReport = (session) => {
    const palette = session.preferences?.palette || 'mono';
    const titleByPalette = {
      mono: '造梦浪漫型人', earth: '自然松弛派', ocean: '冷感知性派', jewel: '清醒复古派', bright: '灵动撞色派', pastel: '柔和浪漫派',
    };
    const eyebrowByPalette = {
      mono: 'LACE', earth: 'NATURAL EASE', ocean: 'COOL POISE', jewel: 'CLEAR RETRO', bright: 'VIVID PLAY', pastel: 'SOFT ROMANCE',
    };
  ...
  const resolvePersonalityPayload = (payload = {}) => {
    ...
    return { ...base, ...payload, ...personalization, heroImage: personalization.heroImage || payload.heroImage || base.heroImage };
  };
  ...
  const renderReport = (payload = {}) => {
    ...
    const usesReferenceCover = !fullHero && data.title === '造梦浪漫型人' && data.eyebrow === 'LACE';
    reportNodes.hero.classList.toggle('report-hero--full', fullHero);
    reportNodes.hero.classList.toggle('report-hero--reference', usesReferenceCover);
    ...
    reportNodes.illustration.closest('figure').hidden = usesReferenceCover || !data.illustration.imageUrl;
    ...
    const proof = reportNodes.source.closest('.report-proof');
    ...
    proof.hidden = !(data.source.name || data.source.copy || data.source.avatars.imageUrl);
    ...
    const outfitCards = data.outfits.filter((item) => item && item.imageUrl).map((item) => {
      const figure = document.createElement('figure');
      const image = Object.assign(document.createElement('img'), {
        src: item.imageUrl, alt: item.alt || item.name || '', loading: 'lazy', decoding: 'async',
      });
  ...
  if (previewParams.get('preview') === 'report') {
    const requestedType = previewParams.get('type');
    if (requestedType) renderReport({ typeId: requestedType });
    showScreen('report');
    shell.classList.add('is-ready');
    return;
  }
  if (previewParams.get('preview') === 'loading') {
