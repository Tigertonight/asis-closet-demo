...
const loader = source.slice(source.indexOf('  let wardrobeRequest'), source.indexOf('  let homeNotesRequest')) +
  source.slice(source.indexOf('  function libraryTopics()'), source.indexOf('  function feedCard(')) +
  source.slice(source.indexOf('  function syncOutfitFavorites()'), source.indexOf('  function loadWardrobe()')) +
  source.slice(source.indexOf('  async function loadFeed('), source.indexOf('  async function load()'));
...
(async () => {
  const state = {feed: [], items: [], outfits: [], topics: [], savedNotes: [], homeOutfits: [], reportOutfits: [], modelGender: "female"};
  const calls = [];
  let topicsFail = false;
  const context = vm.createContext({state, Date, Promise, Boolean, Math, normalizeOutfit: normalized,
    ...
    api: async (url, options) => {
      ...
      throw Error('Unexpected library dependency: ' + url);
    },
  });
  vm.runInContext(loader, context);
  await vm.runInContext('loadFeed()', context);
  assert.equal(state.topics.length, 20);
  assert.equal(state.feed.length, 78, 'structured collections supply all library entries');
  ...
  const firstPage = vm.runInContext('inspiration()', context);
  assert.equal((firstPage.match(/data-try=/g) || []).length, 12);
  assert(firstPage.includes('data-inspiration-next'));
  state.inspirationLimit = 1000;
  ...
  assert.equal((expanded.match(/data-try=/g) || []).length, 65, 'deduplicate while keeping female curvy variants');
  assert.equal((expanded.match(/>\#静音时髦型人<\/span>/g) || []).length, 5, 'female curvy variants retain the same personality');
  for (const id of ['persona-0-0', 'persona-curvy']) {
  ...
  assert(!expanded.includes('data-detail="persona-male"'));
  state.loading = true; state.libraryLoading = true;
  assert(vm.runInContext('inspiration()', context).includes('data-try='), 'already loaded cards stay visible during background startup/refresh');
  state.loading = false; state.libraryLoading = false;
  ...
  assert.equal((withMale.match(/data-try=/g) || []).length, 5, 'male variants stay out of female topic details');
  state.libraryGender = 'male';
  const onlyMale = vm.runInContext('topic()', context);
  assert(onlyMale.includes('男生穿搭') && !onlyMale.includes('微胖穿搭') && !onlyMale.includes('风格穿搭'));
  const maleLanding = vm.runInContext('inspiration()', context);
  assert(!maleLanding.includes('data-detail="persona-curvy"'));
  assert(maleLanding.includes('data-detail="persona-male"'));
