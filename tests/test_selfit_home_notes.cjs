...
(async () => {
  ...
  const calls = [];
  let failed = false;
  const rows = Array.from({length:6}, (_, i) => ({outfit_id:`real-note-${i}`, title:`笔记 ${i}`,
  ...
  const context = vm.createContext({state, URLSearchParams, Set, FormData, reference:false,
    ...
    api:async(url,options)=>{
      ...
      assert(url.startsWith('/selfit/try-on/report-outfits/random?'));
  ...
  assert.equal(state.homeOutfits.length,6);
  ...
  assert.equal(calls.length,1,'navigation and repeated load keep the current six notes');
