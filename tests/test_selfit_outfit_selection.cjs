...
assert(ctx.card({id:'library',personalId:'owned'},'outfit').includes('data-active="true"'));
assert(ctx.card({id:'other'},'outfit').includes('data-active="false"'));
state.generating={target:{id:'different'}};assert(ctx.card({id:'library',personalId:'owned'},'outfit').includes('data-active="false"'));
state.generating={target:{id:'owned'}};assert(ctx.card({id:'library',personalId:'owned'},'outfit').includes('正在试穿'));
ctx.reference=true;ctx.params=new URLSearchParams();state.generating=null;
