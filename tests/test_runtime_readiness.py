...
def test_runtime_readiness_endpoint_returns_user_trial_summary() -> None:
    ...
    response = client.get("/asis/runtime-readiness")
...
def test_runtime_smoke_script_exists() -> None:
    ...
    script = Path("scripts/asis_runtime_smoke.py")
    assert script.exists()
    assert "asis/runtime-readiness" in script.read_text(encoding="utf-8")


def test_full_stack_acceptance_script_exists() -> None:
    from pathlib import Path

    script = Path("scripts/asis_full_stack_acceptance.py")
    text = script.read_text(encoding="utf-8")
    assert script.exists()
    assert "--strict" in text
    assert "start_asis_full_stack.sh" in text
    assert "/stylist/chat" in text
    assert "strict_ai_stylist_chat" in text


def test_sync_asis_env_appends_without_overwriting(tmp_path) -> None:
    from scripts.sync_asis_env import sync_env
    ...
    assert "STYLIST_OPENCLAW_CHAT_URL=http://127.0.0.1:18789/api/asis/chat" in text
