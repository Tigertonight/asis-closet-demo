...
function harness() {
  ...
  const ctx=vm.createContext({state,Map,reference:false,esc:s=>s,mediaURL:s=>s,
    mirror:()=>'<main><div class="mirror-stage model"><div class="arch"></div>MODEL</div></main>',render:()=>renders.push(state.photo),load:()=>reloads++,
...
test('model displays before notes finish and downloads only its preview',async()=>{
  ...
  assert.equal(h.run('readyMirror()'),'<main><div class="mirror-stage model"><div class="arch"></div>MODEL</div></main>');
...
test('a late success after the recovery button appears still recovers',async()=>{
  ...
  await h.requests[0].onload();assert.equal(h.run('readyMirror()'),'<main><div class="mirror-stage model"><div class="arch"></div>MODEL</div></main>');
});
