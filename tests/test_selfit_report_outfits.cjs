...
const lookupCode = studio.slice(studio.indexOf('  function lookup(id)'), studio.indexOf('  function updateWardrobeBusy('));
...
(async () => {
  ...
  state.homeOutfits = [{id: 'home-note', kind: 'outfit', items: []}];
  ...
  assert.equal(state.current.id, 'home-note', 'normal entry selects the first sampled notebook, not an algorithm flat lay');
  state.source = 'report';
