...
- Inspiration displays only theme collection cards in a two-column grid: the four scene/trend collections followed by one collection for each of the 16 personalities. Each card has one large cover and three small notebook previews. Place the theme name in a light pill at the cover's upper-left; remove the bottom caption row and “主题穿搭 ›” text. Open a collection to browse and try its actual delivered outfits; keep curvy variants inside the corresponding personality collection under “微胖穿搭”. Do not duplicate individual outfits on the library landing page (user correction, 2026-09-09).
- EASE, EDGE, MUTE and WABI also include their supplied male outfits under “男生穿搭” inside each personality collection. Keep the original four-note cover/previews and the separate curvy group; hide empty groups. Untested accounts sample four notes for their gender; tested accounts use the four notes of their latest report, preserving its template variant.
