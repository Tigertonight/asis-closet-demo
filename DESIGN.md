...
- The mirror's compact recommendation header shows “今日推荐” on the left and gray, 16px “换一批” text on the right, with no pill, border or background and a minimum 44px touch target. Restore history below the device-link action in the upper-right mirror area, using the original textured clock asset with a small 12px “试穿记录” caption beneath it. Keep the mirror/photo dimensions unchanged and retain compact pagination, navigation clearance and short-screen scrolling (confirmed mockup, 2026-09-14).
- Persist the four current recommendations per account across navigation, reload and reopening. The initial batch uses the latest completed report and its male/curvy template; untested accounts sample gender-filtered delivered notes. Only “换一批” advances to an exploratory batch from all delivered notes of the same gender, including other personalities. Deduplicate notebook photos, avoid seen notes until the pool cycles and avoid the immediately previous batch when possible. A new report/retest or gender change resets this state. Refresh changes only the recommendation list, never the selected outfit, photo, result, pending generation or personality report. Disable duplicate refresh requests and retain the previous list on failure.
- “试穿记录” opens the dedicated “试穿历史” page, with a 44px back button to the mirror and no bottom navigation. Use a warm light background, a two-column grid of rounded cards, complete-image fitting and the local creation date below each photo. Reuse the account history endpoint, newest first. Include loading, empty and retry states. Opening a card enters the large viewer; closing it returns to history, including after reload or browser back/forward.
...
- Inspiration keeps only the four scene/trend collections (通勤场景、约会场景、度假场景、潮流趋势) in a two-column grid. Below them, flatten all personality collections into individual notebook cards in two columns, including curvy variants within the account’s gender. Men see only explicitly male outfits; the legacy `unisex` notebook delivery belongs to the female catalog. Never mix genders in recommendations (user correction, 2026-09-13). Preserve notebook/outfit identities, favorite and direct try-on actions, deduplicate repeated outfit IDs, and load offscreen photos lazily. Do not repeat scene-only notes in this flat list. Each remaining collection keeps its cover, three previews and upper-left theme pill (user correction, 2026-09-13).
- Inspiration notebooks render in batches of 12. Within one screen of the list end, preload only the next batch’s WebP images (at most four warm-up requests at a time) and append cards without replacing existing nodes or shifting scroll. Keep a keyboard/manual load-more fallback and restore the expanded list and scroll after opening a detail. Changing gender resets the batches (user correction, 2026-09-13).
- Inspiration renders as soon as its gender-filtered catalog arrives. Load personal favorites independently; do not wait for unrelated legacy recommendations. Changing gender clears library content and invalidates prior requests.
- Historical personality-collection links remain accessible filtered to the account’s gender, retaining the applicable “微胖穿搭” or “男生穿搭” group; the inspiration landing page no longer shows personality collection cards. Untested accounts still sample four notes for their gender on the mirror; tested accounts use the four notes of their latest report, preserving its template variant.
- The mirror's link icon opens the device-code dialog: title “绑定你的智能穿衣镜”, description “每日记录身材和穿搭”, and a single four-digit numeric input with placeholder “请输入设备码”. Preserve leading zeros and reject other characters. The upper-right “试穿记录” action remains available after try-on and while generating.
...
- “更换照片” opens the native photo picker directly within the user click. Keep the file input inside the active upload dialog so modal inertness cannot block it; cancelling or rejecting a file leaves a visible “选择照片” entry. Preserve the current photo and outfit until a valid replacement is decoded (2026-09-14).
...
- Before fixed-model generation, resolve its current master URL by model ID and fetch original bytes with browser caching disabled. Never submit a stale page photo or a WebP display derivative as the fixed-model original; stop with retry copy if the current model cannot be resolved. Personal uploads keep their selected original (2026-09-14).
...
- Historical own-photo reads request a WebP preview and keep its generation source separate; retrieve the original only when starting try-on. The model chooser shows fixed models as soon as their list is ready, while the optional own-photo preview loads independently; the carousel includes an equally sized “我的照片” card showing the personal preview or a clickable upload placeholder when absent. Keep the bottom personal-photo action. Late completion updates only the personal card and footer action and preserves the carousel position (2026-09-13).
...
- The live mirror loading artwork uses proportional `contain` fitting, centered in the responsive hero: never stretch the “loading / 正在试穿” lettering when hero height changes (2026-09-14).
- The live mirror's textile SVG proportionally covers the hero background at every viewport size, anchored at the upper-left to retain the pin artwork. Changing hero height or wrapping recommendation copy must not expose blank side bands or stretch the artwork; desktop retains its centered mobile shell (2026-09-13).
- Live mirror portraits (width < height, including rounded 3:4 previews and 4:5 photos) use the full available mirror height before and after try-on, including comparison. Scale proportionally, retain the complete vertical extent and clip excess width inside the arch. Square, landscape or undecoded photos retain complete-image `contain` fitting. Hide the separate empty arch when a photo is shown and never expose a solid letterbox. A display-only preview may remove a confidently detected, uniform four-sided image border. Generated results may also have lightly textured, symmetric left/right or top/bottom white bands: trim only when both boundaries are sharp and nearly continuous, preserving the full extent on the other axis. Gradients, edge-touching subjects and uncertain borders retain the original pixels. Preview failure falls back to the original. Keep uploads, generation inputs, history and downloads unchanged. In-place completion displays the result directly without the “试穿图已经好了…” toast.
...
- The default mirror strip shows exactly four delivered notebook photos with their real outfit items, using the persistent recommendation rules above. Report handoff may restore the explicitly chosen outfit and migrate old asset identities, but must not overwrite an already refreshed recommendation batch; selection and recommendations are independent. The personal wardrobe keeps its own outfits. Missing report content shows recovery copy instead of unrelated random notes.
...
- Replace the wardrobe header's solid plus button with a wide, translucent upload dock above the bottom navigation: a large plus and “添加更多你的衣服”, a light dashed border and rounded corners. Show it in both wardrobe tabs; reuse the existing file picker/import flow. Reserve its height outside the list scroll area, account for the bottom safe area, and keep background try-on completion notices above it. The dock disappears outside the wardrobe and while the wardrobe initially loads (user correction, 2026-09-14).
...
- Each right-side deconstructed preview shows the complete matched outfit: the user's white T (or other anchor garment) plus every retained notebook piece. Reuse the shared garment layout, preserve every silhouette, and update the selected outfit when a different notebook row is chosen. The displayed pieces must match the saved/try-on item IDs (user correction, 2026-09-11; placement updated 2026-09-13).
