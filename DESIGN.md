...
- Suit copy follows the supplied warm prototype: warm brown headings, muted brown helper text, photo-purpose subtitles, privacy reassurance, and a separate manual-entry heading/helper/action. “手动选择” and its arrow use the same muted brown as the adjacent helper text.
- Suit explains why photos help. Keep each photo's upload/check status on the upload screen. Once both face and body uploads are accepted, enable “帮我识别” and stay on that screen; only clicking the button opens the dedicated processing screen and requests the suit summary. A failed photo stays on the upload screen for retry. A failed summary request returns to the upload screen with both accepted photos retained for another button click. Accepted photos and server-resolved face / skin / body observations appear together. Unknown attributes stay unknown and offer manual selection.
