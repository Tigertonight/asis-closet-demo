(() => {
  ...
  const fixtureOutfits = Array.from({ length: 9 }, (_, i) => ({
    ...
    name: "灵感套装",
    raw: {can_delete:true},
    src: `${A}main-app/outfit-reference-${i}.webp`,
    items: fixtures,
    saved: i === 5,
  }));
  const feedFixtures = [
  ...
  function mediaURL(path, original = false) {
    ...
    try {
      ...
      return window.SelfitImageURL ? window.SelfitImageURL(url.href, { original }) : url.href;
    } catch {
      return "";
    }
  }
  const state = {
    ...
    profile: null, profileLoading: false, profileError: "", profilePhotos: {}, profileSaving: false, profileDraft: null, profileManualChanges: {}, profileFeatureTouched: false, profilePhotoDraft: {}, profileSuit: null, profileReplacing: "",
    ...
    photo: reference ? `${A}main-app/mirror-reference-photo.webp` : "",
  ...
  async function api(path, options = {}, timeoutMs = 30000) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        const rateLimitMessage = r.status === 429
          ? (gateDetail || (typeof data?.error?.message === 'string' ? data.error.message : '访问太频繁了，请稍后再试。')) +
            (Number.isFinite(retryAfter) && retryAfter > 0 ? ` 请约 ${Math.ceil(retryAfter / 60)} 分钟后重试。` : '')
          : '';
        const e = Error(
          r.status === 401
            ? "请先登录，再查看你的衣橱。"
  ...
  function nav() {
    ...
    $("#navigation").innerHTML = [
      ...
      .join("") + `<button class="nav-ai" data-action="open-chat" aria-label="AI 搭配对话">${image(`${A}main-app/nav-ai-lace.webp?v=20260910`,"")}</button>`;
  }
  ...
  function mirror() {
    ...
    return `<section class="mirror-screen ${styling ? "styling-mode" : "model-mode"}" aria-label="试衣镜"><div class="mirror-stage ${styling ? "styling" : ""} ${pending ? "is-generating" : ""} ${state.result ? "has-result" : ""} ${framedPhoto ? "has-framed-photo" : ""}" aria-busy="${Boolean(pending)}">${image(styling ? `${A}mirror-styling-background.webp?v=20260908-aligned` : `${A}main-app/mirror-background.webp`, "", "stage-background")}<button class="mirror-profile-card" data-page="profile" aria-label="我的档案，查看风格报告"><span class="profile-card-title" aria-hidden="true">my<br>style</span><span>我的档案</span></button><div class="arch"></div>${
      styling && !reference
        ? livePieces()
        : styling
  ...
  function preparePiece(src, retainFineEdges=false) {
    ...
    img.onload = ()=>{
      try {
        ...
        crop.toBlob(blob=>{if(!blob || done)return finish(src,crop.width/crop.height,inkRatio);const url=URL.createObjectURL(blob);state.uploadURLs.push(url);finish(url,crop.width/crop.height,inkRatio);},"image/webp",.92);
      } catch { finish(src); }
    };
  ...
  function mirrorPairArt(className = "") {
    return `<div class="wardrobe-empty-art ${className}" aria-hidden="true"><span class="empty-mirror empty-mirror--back">${image('/static/selfit/assets/lace-card@4x.webp', '', 'empty-mirror-frame')}${image(asset('butterfly'), '', 'empty-mirror-butterfly')}</span><span class="empty-mirror empty-mirror--front">${image('/static/selfit/assets/lace-card@4x.webp', '', 'empty-mirror-frame')}${image(`${A}closet-shirt.webp`, '', 'empty-mirror-shirt')}</span></div>`;
  }
  const chatPrompts = [
  ...
  function closet() {
    if (state.loading || state.wardrobeLoading)
  ...
  function topic() {
    ...
    const groups = hasCurvy || hasMale ? [
      {title:"风格穿搭", entries:selected.entries.filter(entry=>entry.raw?.gender !== "male" && entry.raw?.body_profile !== "curvy")},
      {title:"微胖穿搭", entries:selected.entries.filter(entry=>entry.raw?.gender !== "male" && entry.raw?.body_profile === "curvy")},
      {title:"男生穿搭", entries:selected.entries.filter(entry=>entry.raw?.gender === "male")},
    ].filter(group=>group.entries.length) : [{title:"", entries:selected.entries}];
  ...
  function profileArt(field, value) {
    ...
    return image(['face-oval','body-rectangle'].includes(key) ? `${A}main-app/archive-${key}.webp` : `/static/selfit/assets/manual-selection/${key}@4x.webp`, '', 'profile-attribute-art');
  }
  ...
  function profilePhoto(kind, editable=false) {
    ...
    const photo=state.profilePhotos[kind];
    const loading=!!photo?.loading && !src;
    const failed=!!photo?.error && !src;
    const content=src ? image(src,label) : `<span class="profile-photo-empty" ${loading ? 'role="status"' : ''}>${label}<small>${loading ? '照片加载中…' : failed ? '暂时无法显示' : '还未上传'}</small>${failed ? '<small>点击重试</small>' : ''}</span>`;
    let control;
    // 与 onboarding 的 suit 页一致：照片本身就是替换入口，点击直接换图。
    if(failed) control=`<button type="button" class="profile-photo profile-photo-retry ${editable ? 'edit-photo' : ''}" data-action="reload-profile-photo" data-kind="${kind}" aria-label="重新加载${label}">${content}</button>`;
    else if(editable) control=`<label class="profile-photo edit-photo">${content}${loading ? '' : '<span class="profile-photo-plus" aria-hidden="true">＋</span>'}<input type="file" accept="image/*" data-profile-photo="${kind}" aria-label="更换${label}" ${state.profileSaving || loading ? 'disabled' : ''}></label>`;
    else control=`<label class="profile-photo profile-photo-replace ${replacing ? 'is-replacing' : ''}" title="更换${label}">${content}${src ? '<span class="profile-photo-swap" aria-hidden="true">＋</span>' : ''}<input type="file" accept="image/*" data-profile-photo="${kind}" aria-label="更换${label}" ${replacing || loading ? 'disabled' : ''}></label>`;
    return `<div class="profile-photo-slot" data-profile-photo-slot="${kind}" data-editable="${editable}">${control}</div>`;
  }
  function refreshProfilePhoto(kind) {
    if(!['profile','profile-edit'].includes(state.page)) return;
    // Photo completion must not rerender the report or interrupt an editor/scroll.
    document.querySelectorAll(`[data-profile-photo-slot="${kind}"]`).forEach(slot => {
      slot.outerHTML=profilePhoto(kind, slot.dataset.editable==='true');
    });
  }
  ...
  async function loadProfilePhoto(kind) {
    const photo=state.profilePhotos[kind];
    if(!photo?.source || photo.loading) return;
    photo.loading=true;photo.error=false;
    refreshProfilePhoto(kind);
    try {
      const response=await fetch(photo.source,{headers:{Authorization:`Bearer ${savedSession.accessToken}`,Accept:'image/webp,image/*'}});
      if(response.status===204 || response.status===404) {
        if(state.profilePhotos[kind]===photo && state.profile) {state.profile.photos[kind]=null;photo.source='';}
      } else {
        if(!response.ok) throw new Error('照片暂时无法加载');
        const blob=await response.blob();
        // A refreshed profile or replacement photo takes precedence over older requests.
        if(state.profilePhotos[kind]!==photo || !state.profile) return;
        const url=URL.createObjectURL(blob);
        state.uploadURLs.push(url);state.profile.photos[kind]=url;
      }
    } catch {
      if(state.profilePhotos[kind]===photo) photo.error=true;
    } finally {
      if(state.profilePhotos[kind]===photo) {photo.loading=false;refreshProfilePhoto(kind);}
    }
  }
  async function loadProfile(force=false) {
    ...
    if(reference) {
      state.profile={tested:true,revision:1,gender:params.get('profile_gender')==='male'?'male':'female',manual:{faceShape:'椭圆脸',skin:'中性自然肤',bodyShape:'矩型'},photos:{face:`${A}main-app/archive-face-reference.webp`,body:params.get('profile_gender')==='male'?`${A}main-app/archive-body-reference.webp`:'/static/selfit/assets/samples/female-body-sample-v2.webp'},report:{reportId:'reference',typeId:'flou',title:'造梦浪漫',heroImage:{src:'/static/selfit/assets/personality/flou/hero.webp?v=20260907-config-v2'}},suit:REFERENCE_PROFILE_SUIT};
    ...
    try {
      ...
      const sources=p.photos || {};
      state.profile={...p,photos:{face:null,body:null}};
      state.profileSuit=p.suit || null;
      state.profilePhotos=Object.fromEntries(['face','body'].map(kind=>[kind,{source:sources[kind],loading:false,error:false}]));
      // Fetch both photos independently; the profile request never waits for them.
      for(const kind of ['face','body']) void loadProfilePhoto(kind);
  ...
  async function saveProfile() {
    ...
    const manual=Object.fromEntries(Object.entries(state.profileManualChanges || {}).filter(([,v])=>v));
    if(!Object.keys(manual).length && !Object.keys(state.profilePhotoDraft).length) {state.profileError='请至少选择一项档案信息。';render();return;}
    if(reference) {state.profile.manual={...state.profile.manual,...manual};state.profile.manualOverrides={...state.profile.manualOverrides,...manual};state.profileManualChanges={};Object.entries(state.profilePhotoDraft).forEach(([k,v])=>state.profile.photos[k]=v.url);state.profilePhotoDraft={};go('profile');notify('已更新预览档案');return;}
    const hasPhotoChanges=Object.keys(state.profilePhotoDraft).length > 0;
    state.profileSaving=true;state.profileError='';render();
    try {
      for(const [kind,entry] of Object.entries(state.profilePhotoDraft)) {
        await uploadProfilePhoto(kind, entry.file, entry.url);
        state.profilePhotos[kind]=null;
        state.profile.photos[kind]=entry.url;
        delete state.profilePhotoDraft[kind];
      }
      const current=hasPhotoChanges ? await api('/api/v1/selfit/me/profile') : {profile:state.profile};
      const saved=Object.keys(manual).length ? await api('/api/v1/selfit/me/profile',{method:'PATCH',headers:{'If-Match':String(current.profile.revision)},body:JSON.stringify({reportId:state.profile.report.reportId,manual})}) : current;
      state.profile={...saved.profile,photos:state.profile.photos};state.profileSuit=saved.profile.suit || null;state.profileDraft=null;state.profileManualChanges={};go('profile');notify('档案已保存');
  ...
  function resultViewer() {
    ...
    if (!reference || state.file) return `<section class="result-viewer is-photo" aria-label="试穿大图">${image(`${A}main-app/mirror-background.webp`,'','result-viewer-background')}<div class="result-viewer-photo-area">${image(src,state.viewerPhoto ? '当前模特大图' : '试穿结果大图','result-viewer-photo')}</div><button class="viewer-collapse" data-action="close-viewer" aria-label="收起大图，${returnLabel}">${image(`${A}main-app/result-viewer-collapse.svg`,'')}<span>收起大图</span></button></section>`;
    return `<section class="result-viewer" aria-label="试穿大图">${image(`${A}main-app/result-viewer-background.webp`,'','result-viewer-background')}${reference && !state.file ? image(`${A}main-app/result-viewer-model.webp`,state.viewerPhoto ? '当前模特大图' : '原稿试穿示例','result-viewer-reference') : image(src,state.viewerPhoto ? '当前模特大图' : '试穿结果大图','result-viewer-photo')}<button class="viewer-collapse" data-action="close-viewer" aria-label="收起大图，${returnLabel}">${image(`${A}main-app/result-viewer-collapse.svg`,'')}</button></section>`;
  }
  ...
  function render() {
    ...
    if (builderScroll != null && $(".outfit-builder")) $(".outfit-builder").scrollTop = builderScroll;
    ...
    if (suitHost && window.SelfitSuitCards) {
      ...
      const overrides = state.profile?.manualOverrides || {};
      const features = suit?.features?.length ? suit.features : [
        {key:'skin',title:'肤色',value:manual.skin,source:overrides.skin?'manual':'unknown'},
        {key:'faceShape',title:'脸型',value:manual.faceShape,source:overrides.faceShape?'manual':'unknown'},
        {key:'bodyShape',title:'身材比例',value:manual.bodyShape,source:overrides.bodyShape?'manual':'unknown'},
      ];
      window.SelfitSuitCards.render(suitHost, {features, analyses:(suit && suit.analyses) || {}, photos:(suit && suit.photos) || {}, onEdit:(key)=>{
        ...
        state.profileManualChanges={};state.profileFeatureTouched=false;
  ...
  function go(page, push = true, acceptCompleted = true) {
    ...
    if(page === 'profile' && state.page === 'profile-edit' && !state.profileSaving) {state.profileDraft=null;state.profilePhotoDraft={};state.profileManualChanges={};state.profileFeatureTouched=false;}
  ...
  function builderComposition(pieces) {
    ...
    const measured=pieces.map(piece=>({...piece,
      mirrorCategory:layout.category(piece) || 'accessory',
      aspect:trimmedPieces.get(piece.src)?.aspect,
  ...
  async function loadModels() {
    ...
    filterModelsByGender(state.modelGender);
  }
  function filterModelsByGender(gender) {
    state.modelGender = gender === "male" ? "male" : "female";
    state.modelLibrary = state.modelCatalog.filter(model => model.gender === state.modelGender);
  }
  ...
  async function poll(completedJob = null) {
    ...
    try {
      ...
      if (job.status === "failed") {
        sessionStorage.removeItem("selfit.studio.job");
        throw Error(job.error?.message || job.result?.decision?.user_message ||
          "这次试穿没有完成，请重新尝试。你选择的照片和搭配都已保留。");
      }
      if (!state.generating) {
  ...
  let personalPhotoRequest = null;
  async function loadPhoto() {
    if (state.personalPhoto) return;
    if (personalPhotoRequest) return personalPhotoRequest;
    personalPhotoRequest = (async () => {
      const controller = new AbortController();
      ...
      try {
        ...
        clearTimeout(timer);
        state.personalPhoto ||= state.personalPhotoFallback || "";
      }
    })();
    try { await personalPhotoRequest; } finally { personalPhotoRequest = null; }
  }
  // Share concurrent requests, but refresh favorites on the next explicit reload.
  let inspirationNotesRequest = null;
  function requestInspirationNotes() {
    if (!inspirationNotesRequest) inspirationNotesRequest = api("/selfit/try-on/inspiration-notes")
      .finally(() => { inspirationNotesRequest = null; });
  ...
  function ensureHomeNotes() {
    ...
    homeNotesRequest = loadHomeNotes().finally(() => {
      state.homeNotesLoaded = true;
      state.homeNotesLoading = false;
      homeNotesRequest = null;
      if (!state.loading && state.page === "mirror" && state.source === "inspiration") render();
    });
    return homeNotesRequest;
  }
  function syncOutfitFavorites() {
    for (const row of [...state.feed, ...state.homeOutfits, ...state.reportOutfits, ...(state.topics || []).flatMap(topic => topic.entries)]) {
      const ids = row.items.map(item => item.id).sort().join("|");
      const own = state.outfits.find(x => x.id === row.id || (ids && x.items.map(item => item.id).sort().join("|") === ids));
      if (own) { row.saved = own.saved; row.personalId = own.id; }
    }
  }
  function loadWardrobe() {
    if (wardrobeRequest) return wardrobeRequest;
    if (state.wardrobeLoaded || reference) return Promise.resolve();
    state.wardrobeLoading = true;
    wardrobeRequest = (async () => {
      const results = await Promise.allSettled([
        api("/selfit/try-on/wardrobe"), requestInspirationNotes(),
      ]);
      state.wardrobeError = results[0].status === "rejected" ? "衣帽间暂时无法加载，请重试。" : "";
      state.savedNotesError = results[1].status === "rejected" ? "收藏笔记暂时无法加载，请重试。" : "";
      if (results[0].status === "fulfilled") {
        state.items = (results[0].value.items || []).map(normalizeItem);
        state.outfits = (results[0].value.outfits || []).map(x => normalizeOutfit(x, state.items));
      }
      if (results[1].status === "fulfilled") state.savedNotes = (results[1].value.saved_notes || []).map(x => ({
  ...
  function loadPageData() {
    if (reference || state.loading) return;
    if (state.page === "mirror" && state.source === "inspiration") ensureHomeNotes();
    if (["closet", "builder"].includes(state.page) || (state.page === "mirror" && state.source === "closet")) loadWardrobe();
  ...
  async function prepareInitialModel(preferences) {
    filterModelsByGender(preferences.gender || savedSession?.user?.gender);
    const savedModel = state.modelLibrary.find(x => x.id === preferences.current_model_id);
    ...
    if (!preferred || fromOnboarding) await loadPhoto();
    if (fromOnboarding && state.personalPhoto) {
      preferences.current_model_id = "self";
      api("/selfit/try-on/model", {
    ...
    if (state.personalPhoto && (preferences.current_model_id === "self" || !preferred)) {
      state.modelId = "self";
      state.photo = state.personalPhoto;
      state.file = state.personalFile;
    } else {
      const model = preferred || defaultModel;
      state.modelId = model?.id || "";
      state.photo = model?.image_url || "";
      state.file = null;
    }
  ...
  async function load() {
    ...
    try {
      ...
      const initial = await Promise.allSettled([modelReady, outfitsReady]);
  ...
  $("#studio").addEventListener("click", async (e) => {
    ...
    if (b.dataset.profileChoice) {state.profileFeatureValue=b.dataset.profileChoice;state.profileFeatureTouched=true;render();return;}
    if (b.dataset.action==='cancel-profile-feature' || b.dataset.action==='confirm-profile-feature') {
      if(b.dataset.action==='confirm-profile-feature') {
        if(!state.profileFeatureTouched || !state.profileFeatureValue) return;
        state.profileManualChanges[state.profileEditingField]=state.profileFeatureValue;
    ...
    try {
      ...
      if(b.dataset.action === "edit-profile") {state.profileDraft={...state.profile.manual};state.profileManualChanges={};state.profilePhotoDraft={};state.profileError='';go('profile-edit');return;}
      if(b.dataset.action === "reload-profile") {await loadProfile(true);return;}
      if(b.dataset.action === "reload-profile-photo") {await loadProfilePhoto(b.dataset.kind);return;}
      ...
      switch (b.dataset.action) {
        ...
        case "save-result": {
          ...
          const response = await fetch(mediaURL(state.result, true));
  ...
  $("#studio").addEventListener("change", async (event)=>{
    ...
    if(field) {state.profileManualChanges[field]=event.target.value;state.profileDraft={...(state.profileDraft || state.profile.manual),[field]:event.target.value};render();return;}
