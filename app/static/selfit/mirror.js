(() => {
  ...
  const processPhoto=async()=>{
    ...
    if(runId!==analysisRun)return;later(()=>{if(runId!==analysisRun)return;show('result');const qrUrl=responseData?.qrImageUrl||config.qrImageUrl;retryQrCode.hidden=Boolean(qrUrl);qrImage.hidden=!qrUrl;if(qrUrl){track('mirror_qr_generated',{handoffId:responseData?.handoffId||null});qrImage.src=qrUrl;qrTitle.textContent='手机扫码继续';const expiresAt=Date.parse(responseData?.expiresAt||'');const updateExpiry=()=>{if(app.dataset.state!=='result')return;const remaining=Math.max(0,Math.ceil((expiresAt-Date.now())/1000)),minutes=String(Math.floor(remaining/60)).padStart(2,'0'),seconds=String(remaining%60).padStart(2,'0');qrCode.textContent=remaining>0?`${minutes}:${seconds} 内有效`:'二维码已过期';if(remaining>0)later(updateExpiry,1000);else{track('mirror_qr_expired',{handoffId:responseData?.handoffId||null});qrImage.hidden=true;retryQrCode.hidden=false;}};updateExpiry();const pollStatus=async()=>{if(app.dataset.state!=='result'||!responseData?.statusUrl)return;try{const response=await fetch(responseData.statusUrl,{cache:'no-store'}),payload=await response.json();if(payload.handoff?.status==='claimed'){track('mirror_qr_claim_detected',{handoffId:responseData?.handoffId||null});qrTitle.textContent='已保存到手机';qrCode.textContent='手机上将继续 like 与 vibe';qrImage.classList.add('is-claimed');later(reset,2600);return;}}catch{}later(pollStatus,1500);};later(pollStatus,1500);}else{qrTitle.textContent='生成失败';qrCode.textContent='检查网络后重新生成';}busy=false;delete app.dataset.processingStage;later(reset,Number(config.idleTimeoutMs)||60000);},Math.max(3500,config.minimumAnalysisMs||0));
  };
