(() => {
  ...
  // 镜子端埋点：fire-and-forget，无登录态（匿名 + IP）
  const track = (event, props = {}) => {
    try {
      const payload = { events: [{ event, props }] };
  ...
  const qrRetryDelays=[1500,3000,5000,10000,15000];
  const clearQrObjectUrl=()=>{if(qrObjectUrl)URL.revokeObjectURL(qrObjectUrl);qrObjectUrl='';};
  const retryAfterMs=(response)=>{const value=response.headers.get('Retry-After');if(!value)return 0;const seconds=Number(value);if(Number.isFinite(seconds))return Math.max(0,seconds*1000+1000);const date=Date.parse(value);return Number.isFinite(date)?Math.max(0,date-Date.now()+1000):0;};
  const showQrBlob=(blob)=>new Promise((resolve,reject)=>{if(!blob.type.startsWith('image/')){reject(new Error('invalid qr content type'));return;}const objectUrl=URL.createObjectURL(blob),probe=new Image();probe.onload=()=>{clearQrObjectUrl();qrObjectUrl=objectUrl;qrImage.src=objectUrl;qrImage.classList.remove('is-loading');qrImage.hidden=false;resolve();};probe.onerror=()=>{URL.revokeObjectURL(objectUrl);reject(new Error('invalid qr image'));};probe.src=objectUrl;});
  const loadQrImage=async({url,expiresAt,runId,attempt=0})=>{if(runId!==analysisRun||app.dataset.state!=='result')return;qrImage.hidden=false;qrImage.classList.add('is-loading');qrTitle.textContent=attempt?'二维码加载失败，正在自动重试':'二维码加载中';try{const response=await fetch(url,{cache:'no-store'});if(!response.ok){const error=new Error(`qr ${response.status}`);error.retryAfter=retryAfterMs(response);throw error;}await showQrBlob(await response.blob());if(runId!==analysisRun||app.dataset.state!=='result')return;qrTitle.textContent='手机扫码继续';if(attempt)track('mirror_qr_retry_succeeded',{attempt});}catch(error){if(runId!==analysisRun||app.dataset.state!=='result')return;const remaining=Math.max(0,expiresAt-Date.now());if(remaining<=1000){qrTitle.textContent='二维码已过期';qrImage.hidden=true;retryQrCode.hidden=false;return;}const fallback=qrRetryDelays[Math.min(attempt,qrRetryDelays.length-1)],requested=Number(error.retryAfter)||0,delay=Math.min(Math.max(fallback,requested),Math.max(1000,remaining-500));track('mirror_qr_retry_scheduled',{attempt:attempt+1,delayMs:delay});later(()=>void loadQrImage({url,expiresAt,runId,attempt:attempt+1}),delay);}};
  const processPhoto=async()=>{
    if(busy)return;busy=true;clearTimers();show('processing');track('mirror_capture_confirmed');const runId=++analysisRun,renderStage=(stage)=>{if(runId===analysisRun)renderProcessingStage(stage);};renderStage(processingStages[0]);processingStages.slice(1).forEach((stage)=>later(()=>renderStage(stage),stage.delay));let responseData=null;
    ...
    if(runId!==analysisRun)return;later(()=>{if(runId!==analysisRun)return;show('result');const qrUrl=responseData?.qrImageUrl||config.qrImageUrl;retryQrCode.hidden=Boolean(qrUrl);qrImage.hidden=!qrUrl;if(qrUrl){track('mirror_qr_generated',{handoffId:responseData?.handoffId||null});const parsedExpiry=Date.parse(responseData?.expiresAt||''),expiresAt=Number.isFinite(parsedExpiry)?parsedExpiry:Date.now()+600000;void loadQrImage({url:qrUrl,expiresAt,runId});const updateExpiry=()=>{if(app.dataset.state!=='result')return;const remaining=Math.max(0,Math.ceil((expiresAt-Date.now())/1000)),minutes=String(Math.floor(remaining/60)).padStart(2,'0'),seconds=String(remaining%60).padStart(2,'0');qrCode.textContent=remaining>0?`${minutes}:${seconds} 内有效`:'二维码已过期';if(remaining>0)later(updateExpiry,1000);else{track('mirror_qr_expired',{handoffId:responseData?.handoffId||null});qrImage.hidden=true;retryQrCode.hidden=false;}};updateExpiry();let pollFailures=0;const pollStatus=async()=>{if(app.dataset.state!=='result'||!responseData?.statusUrl)return;let delay=3000;try{const response=await fetch(responseData.statusUrl,{cache:'no-store'});if(!response.ok){delay=Math.max(delay,retryAfterMs(response));throw new Error(`status ${response.status}`);}const payload=await response.json();pollFailures=0;if(payload.handoff?.status==='claimed'){track('mirror_qr_claim_detected',{handoffId:responseData?.handoffId||null});qrTitle.textContent='已保存到手机';qrCode.textContent='手机上将继续 like 与 vibe';qrImage.classList.add('is-claimed');later(reset,2600);return;}}catch{pollFailures+=1;delay=Math.max(delay,Math.min(30000,3000*(2**Math.min(pollFailures,3))));}later(pollStatus,delay);};later(pollStatus,3000);}else{qrTitle.textContent='生成失败';qrCode.textContent='检查网络后重新生成';}busy=false;delete app.dataset.processingStage;later(reset,Number(config.idleTimeoutMs)||60000);},Math.max(3500,config.minimumAnalysisMs||0));
  };
