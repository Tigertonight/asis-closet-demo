...
async def closet_import_job_create(images: list[UploadFile] = File(...), current_user: dict[str, Any] = Depends(require_beta_user)) -> dict[str, Any]:
    with user_storage(current_user["user_id"]):
        return await create_import_upload_job(images, current_user["user_id"])


@app.get("/closet/import/jobs/{job_id}")
def closet_import_job(job_id: str, current_user: dict[str, Any] = Depends(require_beta_user)) -> dict[str, Any]:
    with user_storage(current_user["user_id"]):
        return get_import_job(job_id)


@app.post("/closet/import/jobs/{job_id}/retry")
def closet_import_job_retry(job_id: str, current_user: dict[str, Any] = Depends(require_beta_user)) -> dict[str, Any]:
    with user_storage(current_user["user_id"]):
...
async def closet_outfit_recommendations(request: Request, current_user: dict[str, Any] = Depends(require_beta_user)) -> dict[str, Any]:
    with user_storage(current_user["user_id"]):
...
def selfit_try_on_job(job_id: str, current_user: dict[str, Any] = Depends(require_beta_user)) -> dict[str, Any]:
    with user_storage(current_user["user_id"]):
