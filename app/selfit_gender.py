...
from app.selfit_onboarding_store import select, find
...
def declared_profile(data: dict, user_id: str) -> dict:
    ...
    for report in select(data, "reports", user_id=user_id):
...
def save_gender(data: dict, user_id: str, gender: str) -> dict:
    ...
    entry = find(data, "user_profiles", user_id=user_id)
    if entry is None:
