...
uniform float adjustable_blur_x;
  ...
  vec4 blur(vec4 curColor, float blurSize, vec2 uv,float intensity){
    ...
    for(int i=1;i<=8;i++)
    {
        vec2 curRightCoordinate = uv+vec2(float(i),float(0))*unit_uv;
        vec2 curLeftCoordinate  = uv+vec2(float(-i),float(0))*unit_uv;
...
void main()
{
    ...
    fragColor = blur(color, 4.8, v_texcoord,adjustable_intensity / 100.0*adjustable_blur_x/100.0);
}
