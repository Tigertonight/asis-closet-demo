...
vec4 blur(sampler2D inTexture, vec4 curColor, float blurSize, vec2 uv, vec2 resolution){
    ...
    for(int i=1;i<=8;i++)
    {
        vec2 curRightCoordinate = uv+vec2(0.0, float(i))*unit_uv;
        vec2 curLeftCoordinate  = uv+vec2(0.0, float(-i))*unit_uv;
...
void main()
{
    vec4 maskColor = texture(_BaseTexture0, v_texcoord);
    fragColor = blur(_BaseTexture0, maskColor, 3.0, v_texcoord, vec2(image_width, image_height));
}
