...
## 当前算法口径（v1.5-male-four）

### 男性候选范围（2026-09-12）

男性新报告暂时只在 **EASE、EDGE、MUTE、WABI** 中选出主、次型格，置信度也以这四个候选的前两名计算。女性和未指定性别的旧调用仍使用完整 16 型。七维输入、中心点、距离与权重不变；这是素材开放范围限制。

后端 `MALE_PERSONA_CODES` 和前端同名常量控制名单，管理后台读取后端候选范围并提供男性四型手动判定。补齐某型男性封面、四篇笔记及拆款映射后，同时扩展两端名单、递增算法版本并通过对拍测试；不自动开放尚未验证的素材。历史报告不批量重算，重新测试后使用新范围。
...
- 对当前开放候选各算加权绝对距离：`Σ wᵢ·|userᵢ - centerᵢ|`（男性 4 型，其余 16 型）
...
`ALGORITHM_VERSION`（当前 `v1.5-male-four`）出现在：
