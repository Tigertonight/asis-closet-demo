...
交付对应 16 组标准人格，以及 FILM、LOOP、VOID、WABI 的 4 组微胖版本。每组 4 条笔记，共 80 条笔记、468 个单品；通过 `templateId + noteId` 独立绑定，微胖版本不会覆盖标准版本。

2026-09-10 更新 FLOU 前两条笔记为「浪漫彩搭」和「薄荷蓝粉」，原 14 个单品替换为 21 个，作者及链接同步更新。23 张新图片均已上传七牛并回读校验，21 张已有透明 PNG 保持原始字节；更新过程、描述和叠穿规范化说明见 [FLOU 两条笔记更新](SELFIT_FLOU_NOTE_UPDATE_20260910.md)。
...
- `app/data/styling-source-assets.v1.json`：交付目录 652 个图片文件与素材 ID 的映射。
...
两个图片清单合计 832 个文件引用，按原始文件内容去重后为 711 张、635,610,292 字节。交付目录本身包含 631 张不同图片，其余 80 张是项目既有报告基础素材。这里统计的是原始拆款与报告图片，当前透明单品绑定另见 `styling-cutout-assets.v1.json`。文件名相同但内容不同的图片不会互相覆盖。

业务数据使用 `assetId`；项目中通过 `/api/v1/material-assets/{assetId}/content` 获取图片。拆款里的 `contentUrl` 是这个固定入口，`url` 是存储对象地址。私有空间的对象地址不能直接匿名访问；维护者使用 `scripts/refresh_material_urls.py` 发布临时链接，普通读取不签名。新增素材提交 Git 时不包含临时签名，运行环境可通过 `SELFIT_MATERIAL_REGISTRY_PATH` 使用 Git 忽略目录中的已刷新清单。
...
现有域名是七牛测试域名，只有 30 个自然日生命周期，不支持 HTTPS 配置；长期使用需绑定正式域名。参见[七牛测试域名规范](https://developer.qiniu.com/fusion/1319/test-domain-access-restriction-rules)。项目对这些已授权发布的素材使用同源图片接口，服务端使用已发布的临时链接下载、校验并缓存到 `outputs/material-cache/`，因此页面无需携带签名，也无需直接加载 HTTP 域名。此素材库不用于用户私有照片。
