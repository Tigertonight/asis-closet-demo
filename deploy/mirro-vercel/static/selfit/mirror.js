(() => {
  ...
  let countdownRun = 0;
  let analysisRun = 0;
  let permissionWarmupPromise = null;
  ...
  const warmCameraPermission = () => {
    if (!navigator.mediaDevices?.getUserMedia) {
      app.dataset.cameraPermission = 'unsupported';
      return Promise.resolve(false);
    }
    if (permissionWarmupPromise) return permissionWarmupPromise;
    app.dataset.cameraPermission = 'prompting';
    permissionWarmupPromise = navigator.mediaDevices.getUserMedia(cameraConstraints)
      .then((permissionStream) => {
        permissionStream.getTracks().forEach((track) => track.stop());
        app.dataset.cameraPermission = 'granted';
        return true;
      })
      .catch((error) => {
        app.dataset.cameraPermission = ['NotAllowedError', 'SecurityError'].includes(error?.name) ? 'denied' : 'unavailable';
        return false;
      })
      .finally(() => { permissionWarmupPromise = null; });
  ...
  const startCamera = async (runId) => {
    ...
    try {
      if (permissionWarmupPromise) await permissionWarmupPromise;
      requestedStream = await navigator.mediaDevices.getUserMedia(cameraConstraints);
  ...
  const runCountdown = async () => {
    ...
    await startCamera(runId);
    if (runId !== countdownRun || app.dataset.state !== 'countdown') return;
